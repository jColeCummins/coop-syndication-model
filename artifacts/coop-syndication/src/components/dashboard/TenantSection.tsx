...
export function TenantSection({ model, tooltips }: { model: DealModel; tooltips: typeof TOOLTIPS }) {
          ...
          <table className="w-full text-right text-[11px] tabular-nums whitespace-nowrap">
            ...
            <tbody className="divide-y divide-border">
              <tr className="hover:bg-muted/30 transition-colors">
                ...
                <td className="py-3 px-4">{formatCurrency(tenant.phase1AnnualDebtService)}</td>
                <td className="py-3 px-4">{formatCurrency(tenant.phase2AnnualDebtService)}</td>
              </tr>
              <tr className="hover:bg-muted/30 transition-colors">
                <td className="py-3 px-4 text-left">Property Taxes</td>
                <td className="py-3 px-4">{formatCurrency(inputs.propertyTaxes)}</td>
                <td className="py-3 px-4">{formatCurrency(inputs.propertyTaxes)}</td>
              </tr>
              <tr className="hover:bg-muted/30 transition-colors">
                <td className="py-3 px-4 text-left">Insurance &amp; Misc</td>
                <td className="py-3 px-4">{formatCurrency(inputs.annualInsuranceMisc)}</td>
                <td className="py-3 px-4">{formatCurrency(inputs.annualInsuranceMisc)}</td>
              </tr>
              <tr className="hover:bg-muted/30 transition-colors">
                <td className="py-3 px-4 text-left">Mgmt</td>
                <td className="py-3 px-4">{formatCurrency(tenant.phase1MgmtAnnual)}</td>
                <td className="py-3 px-4">{formatCurrency(tenant.phase2MgmtAnnual)} <span className="text-[9px] text-muted-foreground ml-1">&times;1.15</span></td>
              </tr>
