...
- Left sidebar: plain-language labels + tooltips on every input (UX-CONFIG-SPEC §2), 36 sliders + 4 switches (MFJ, Ohio BID, pref current-pay, REPS) in 5 groups incl. Grants & Subsidy (4 buckets by award probability, $0 = not awarded, soft-funding treatment, § 465 at-risk enforced) and an optional CRA/restricted-rent property-tax abatement (pct + term, off by default, tenant-rent only); mobile: accordion toggle; state via useReducer
