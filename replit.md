...
- Left sidebar: plain-language labels + tooltips on every input (UX-CONFIG-SPEC §2), 46 sliders + 5 switches (MFJ, Ohio BID, pref current-pay, REPS, rent policy) in 6 groups incl. Grants & Subsidy (4 buckets by award probability, $0 = not awarded, soft-funding treatment, § 465 at-risk enforced), an optional CRA/restricted-rent property-tax abatement (pct + term, off by default, tenant-rent only), Member Shares & Buy-In (share price + participation + assistance ask, priced below the Greenmont benchmark against displacement risk), and Rent Policy & Surplus (policy rent + 3-way surplus split + LTV/DSCR lender tests); mobile: accordion toggle; state via useReducer
- V5.7 adds RENT POLICY + PHASE-2 FEASIBILITY: cost-recovery rent is a floor, not a target — charging it exactly pins DSCR at 1.00 and the takeout is unfinanceable. Policy rent (default on, $700) is a floor on rent (co-op charges the greater of policy and cost floor, so surplus is never negative; below-floor policies are flagged, not silently applied). Surplus splits 3 ways: extra seller-note principal (inside the amortization loop — balloon now comes from the schedule, not closed-form remainingBalance), capital reserves (NOT netted against the refinance), shadow equity (non-redeemable; see A.16 for the lender/§216 caveats). FeasibilitySection tests the refinance against max-LTV AND min-DSCR, reports the supportable loan as the lesser and the shortfall as a dollar financing gap. `rentPolicyEnabled: false` reproduces V5.6 exactly.
- V5.10 corrected PIK: accrual now COMPOUNDS (true paid-in-kind; simple accrual understated it $13,821 at defaults) and runs to the BUYOUT rather than the note term (investors are outstanding past a self-amortizing note). NPV conventions, seller recapture (Reg. 1.453-12 front-loading) and investor exit recapture audited clean — see ANALYSIS.md A.20. investor.purchaseBasis is CONTRACT PRICE ONLY; the depreciable basis is contract price + CapEx, now exposed as exit.depreciableBasis / exit.totalDepreciationTaken.
- V5.11 answers the July 2026 external review (ANALYSIS.md A.21): stabilizedValue is now an OVERRIDE (0 = derive from Phase-2 NOI / exitCapRate, default 7%), because claiming a suppressed value at the county auditor and a market value at the bank was a real internal contradiction. Feasibility panel reports the income-approach value, the implied cap rate of any override, and flags divergence >1.15x (the old $1.45M reads as 1.49x at a 4.71% implied cap). Utilities default raised 1150 -> 1500/unit/yr. Debt-vs-equity recharacterization added to the diligence stack.
- V5.12 FEATURE FREEZE (ANALYSIS.md A.22, docs/CLOSE-OR-RESTRUCTURE.md). Printed base case is now the conservative one: REPS OFF, Ohio BID OFF, reserves $550, vacancy 6% — Phase-1 floor $802 -> $832, Year-1 investor refund $92,050 -> $0 (passive losses suspend under 469; the refund narrative was REPS-only). Added OBBBA CHARITABLE_BENEFIT_CAP 0.35 on ordinary-income offsets (binds above the 35% bracket only). 461(l) checked and does NOT bind — 465 at-risk caps the Year-1 deduction at $263k, below the ~$313k single threshold. Remaining review items are counsel/lender/appraiser work, tracked in the sprint checklist, NOT engine features.
- V5.13 adds `investorsEnabled` MASTER SWITCH (default true). OFF collapses the syndicate entirely — no investor capital, no pref in rent, no bonus depreciation, no capital returned at buyout, so the Phase-2 refinance carries the seller balloon alone; unfunded closing need surfaces as feasibility.closingCapitalGap and the investor card is hidden. Rationale (A.23): a REPS investor already running paper losses has no marginal appetite, and 469 suspends a passive investor's losses until exit, so the tax premise may be theoretical for the actual buyer pool. CORRECTED IN A.24: that headline was a confounded 3-change comparison. Isolated, the CapEx + down-payment cut does essentially all the work — with a small capital need the investor toggle moves the Phase-1 floor $0 (C vs D both $792). What survives is that investor equity is the most expensive capital source (1.35x vs 1.00x for shares/grants, 1.15x for a 3% sub-loan), so source cheap capital first; whether to renovate at closing is a SEPARATE decision and it is the one that moves rent.
- Main view: 6-KPI strip + four data cards: Investor card now leads with a plain-English ROI band (capital in / Year-1 tax refund / effective capital at risk / total back); Seller (§ 170 max-absorbable-donation readout + expiry alert, deal geometry, annual cash & tax schedule with balloon/no-shield badges, 3-scenario cash-sale comparison), Investor (Y1 depreciation build-up, flow table w/ takeout row, exit economics + gain split by rate, IRR/EM/payback, optimal-when bullets), Tenant (rent triptych w/ annualized-drift framing, escalated Phase-1/Phase-2 revenue build-up with per-line escalator badges, note amortization)
